...
autor: Ramiro Gil
fecha: 2026-05-01
titulo: Eliminación de Pago Existente
---

# TDD-0003: Eliminación de Pago Existente
...
| Cancelación exitosa      | Respuesta vacía                               | 204 No Content            |
...
3. Crear el endpoint `DELETE /api/v1/payments/:id` en el `PaymentController` y registrarlo en `app.ts`.
4. Añadir el método `delete` al servicio Frontend (`payments.ts`).
5. Enlazar el botón de eliminación en `PaymentsView.tsx` agregando la confirmación del navegador (`window.confirm`) antes de hacer la llamada.
