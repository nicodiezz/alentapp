...
describe('DisciplineController', () => {
    ...
    describe('create', () => {
        ...
        it('debe devolver status 400 Bad Request si la fecha de fin de suspensión es previa o igual a la fecha de inicio', async () => {
            mockCreateUseCase.execute.mockRejectedValueOnce(new Error('La fecha de fin de suspensión no puede ser previa o igual a la fecha de inicio'));
            
            await controller.create(mockRequest as any, mockReply as any);
            
            expect(mockReply.status).toHaveBeenCalledWith(400);
            expect(mockReply.send).toHaveBeenCalledWith({ error: 'La fecha de fin de suspensión no puede ser previa o igual a la fecha de inicio' });
        });
    ...
    describe('update', () => {
        it('debe devolver status 200 y los datos actualizados si la actualización es exitosa', async () => {
            const mockSuspension = { id: '123', reason: 'Pelea actualizada' };
            mockUpdateUseCase.execute.mockResolvedValueOnce(mockSuspension);
        ...
        it('debe devolver status 404 Not Found si la suspensión no existe', async () => {
            mockUpdateUseCase.execute.mockRejectedValueOnce(new Error('La suspensión no existe'));

            await controller.update(mockRequest as any, mockReply as any);

            expect(mockReply.status).toHaveBeenCalledWith(404);
            expect(mockReply.send).toHaveBeenCalledWith({ error: 'La suspensión no existe' });
        });

        it('debe devolver status 404 Not Found si el miembro indicado no existe', async () => {
            mockUpdateUseCase.execute.mockRejectedValueOnce(new Error('El miembro indicado no existe'));
        ...
        it('debe devolver status 400 Bad Request si la fecha de fin es previa o igual a la fecha de inicio', async () => {
            mockUpdateUseCase.execute.mockRejectedValueOnce(new Error('La fecha de fin de suspensión no puede ser previa o igual a la fecha de inicio'));
        ...
        it('debe devolver status 400 Bad Request si el formato de fecha es inválido', async () => {
            mockUpdateUseCase.execute.mockRejectedValueOnce(new Error('Formato de fecha inválido'));
    ...
    describe('delete', () => {
        it('debe devolver status 204 No Content si la eliminación es exitosa', async () => {
        ...
        it('debe devolver status 404 Not Found si la suspensión no existe', async () => {
            mockDeleteUseCase.execute.mockRejectedValueOnce(new Error('La suspensión no existe'));
